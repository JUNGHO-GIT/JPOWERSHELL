...
class M {
	...
	static [void] DetectStructureMode() {
		...
		if ($null -eq $files) {
			$global:useTopFilesMode = $false; return
  }
		...
		$sorted += $rest
		foreach ($f in $sorted) {
			$global:topFiles += [PSCustomObject]@{ name = $f.Name; path = $f.FullName; extension = $f.Extension }
		}
	...
	static [void] GetFolderList() {
		...
		if (-not $pathExists) {
			[T]::PrintExit("Red", "! js 경로가 존재하지 않습니다: $jsPath"); return
  }
		...
		foreach ($folder in $folders) {
			$files = Get-ChildItem -Path $folder.FullName -File -ErrorAction SilentlyContinue | Where-Object { $_.Extension -eq ".js" -or $_.Extension -eq ".ts" -or $_.Extension -eq ".ps1" }
			$fileCount = ($null -ne $files) ? $files.Count : 0
			[T]::PrintText("White", "- 폴더 발견: $($folder.Name) - 파일: $fileCount 개")
			$global:folderList += [PSCustomObject]@{ number = $number; name = $folder.Name; path = $folder.FullName; fileCount = $fileCount }
			$number++
		}
	}
	...
	static [void] CollectExecutableFiles() {
		foreach ($folder in $global:selectedFolders) {
			$files = Get-ChildItem -Path $folder.path -File -ErrorAction SilentlyContinue | Where-Object { $_.Extension -eq ".js" -or $_.Extension -eq ".ts" -or $_.Extension -eq ".ps1" }
			if ($null -eq $files) {
			...
			foreach ($file in $files) {
				...
				$global:allFiles += [PSCustomObject]@{ name = $file.Name; path = $file.FullName; extension = $file.Extension }
			}
		}
	}
	...
	static [void] RunMultipleFiles() {
		...
		$processes = @(); $outputFiles = @(); $errorFiles = @()
		for ($i = 0; $i -lt $global:allFiles.Count; $i++) {
			...
			$outputFiles += $outputFile; $errorFiles += $errorFile
		...
		[T]::PrintLine("Yellow"); [T]::PrintText("Yellow", "▶ 모든 프로세스가 완료될 때까지 대기 중...")
		foreach ($process in $processes) {
			$process.WaitForExit()
  }
		[T]::PrintText("Yellow", "▶ 모든 프로세스가 완료되었습니다.")
		for ($i = 0; $i -lt $global:allFiles.Count; $i++) {
			[T]::PrintLine("Cyan"); [T]::PrintText("Cyan", "▶ [$($global:allFiles[$i].name)] 실행 결과:"); [T]::PrintLine("White")
			if (Test-Path $outputFiles[$i]) {
				$content = Get-Content $outputFiles[$i] -Raw -ErrorAction SilentlyContinue -Encoding UTF8; if ($content) {
					Write-Host $content
				}
   }
			if (Test-Path $errorFiles[$i]) {
				$errorContent = Get-Content $errorFiles[$i] -Raw -ErrorAction SilentlyContinue -Encoding UTF8; if ($errorContent) {
		...
		[T]::PrintLine("Yellow"); [T]::PrintText("Yellow", "▶ 총 $($processes.Count)개의 프로세스가 실행되었습니다.")
	}

	## JSESSION 입력 공통 처리
	static [void] PromptJSession([string]$displayName, [string]$envVarName, [ref]$targetVar) {
		[T]::PrintLine("Green"); [T]::PrintText("Green", "▶ [$displayName] 설정")
		...
		[T]::TextInput("Green", $jsessionDisplayMsg, $targetVar); [T]::PrintEmpty()
	...
	static [void] PromptPgCode([ref]$targetVar) {
		[T]::PrintLine("Green"); [T]::PrintText("Green", "▶ PG사 선택:")
		[T]::PrintText("White", "- 01: 코페이"); [T]::PrintText("White", "- 02: 모빌"); [T]::PrintText("White", "- 03: 다날"); [T]::PrintText("White", "- 04: 갤컴"); [T]::PrintText("White", "- 05: KIS"); [T]::PrintText("White", "- 06: TOSS"); [T]::PrintText("White", "- 07: KSNET"); [T]::PrintText("White", "- 08: SECTANINE (기본값)"); [T]::PrintText("White", "- 09: SECTANINE-VAN"); [T]::PrintText("White", "- 10: KOCES"); [T]::PrintText("White", "- 11: 다우")
		[T]::PrintLine("Green")
		...
		[T]::TextInput("Green", $pgPromptMessage, $targetVar); [T]::PrintEmpty()
		if ([string]::IsNullOrWhiteSpace($targetVar.Value)) {
...
& {
	if ($global:useTopFilesMode) {
		...
		[T]::PrintEmpty()
		$hasKorpay = $false; $hasSecta = $false
		...
		if ($hasKorpay) {
			[M]::PromptJSession("KORPAY", "KORPAY_JSESSION", [ref]$global:jsessionKorpay); [M]::PromptPgCode([ref]$global:pgCode)
  }
		if ($hasSecta) {
		...
		try {
			[M]::CreateTempOutputDir(); [M]::CleanupTempOutputFiles(); [M]::RunMultipleFiles()
  }
		catch {
			[T]::PrintExit("Red", "! 오류 발생: $_")
  }
		[T]::PrintLine("Cyan"); [T]::PrintText("Cyan", "▶ 모든 작업이 완료되었습니다.")
	}
	else {
		[T]::PrintLine("Cyan")
		[M]::GetFolderList()
		...
		foreach ($folder in $global:folderList) {
			$message = "- $($folder.number). $($folder.name) ($($folder.fileCount)개 파일)"; [T]::PrintText("White", $message)
  }
		...
		[T]::PrintEmpty()
		[T]::PrintLine("Cyan"); [T]::PrintText("Cyan", "▶ 실행 파일 수집 중...")
