...
class M {
	...
	static [void] LoadEnvFile() {
		...
		-not (Test-Path $envFilePath) ? (
			...
			Get-Content $envFilePath | ForEach-Object {
				$line = $_.Trim()
				$line -and -not $line.StartsWith("#") -and $line.Contains("=") ? (
					$parts = $line.Split("=", 2),
					$parts.Count -eq 2 ? (
						$key = $parts[0].Trim(),
						$value = $parts[1].Trim(),
	...
	static [void] CreateTempOutputDir() {
		-not (Test-Path $global:tempOutputPath) ? (
	...
	static [void] CleanupTempOutputFiles() {
		Test-Path $global:tempOutputPath ? (
	...
	static [void] PromptJSession(
		...
		[T]::PrintEmpty()
		[string]::IsNullOrWhiteSpace($targetVar.Value) ? (
	...
	static [void] PromptPgCode([ref]$targetVar) {
		...
		[string]::IsNullOrWhiteSpace($defaultPgCode) ? (
			$defaultPgCode = "08"
		) : (
			$null
		)
		...
		[string]::IsNullOrWhiteSpace($targetVar.Value) ? (
			...
			try {
				...
				$numValue -lt 1 -or $numValue -gt 11 ? (
...
& {
	$global:useTopFilesMode ? (
		...
		foreach ($part in $inputParts) {
			try {
				$num = [int]$part
				$num -lt 1 -or $num -gt $global:topFiles.Count ? (
					[T]::PrintExit("Red", "! 번호 $num 가 범위를 벗어났습니다. (1-$($global:topFiles.Count))")
				) : (
					$null
				)
				$selectedFileIdx -contains $num ? (
		...
		foreach ($file in $global:allFiles) {
			$file.name -eq "korpay.js" ? (
				$hasKorpay = $true
			) : $file.name -eq "secta.js" ? (
				$hasSecta = $true
			) : (
				$null
			)
		...
		foreach ($part in $inputParts) {
			try {
				$num = [int]$part
				$num -lt 1 -or $num -gt $global:folderCount ? (
					[T]::PrintExit("Red", "! 번호 $num 가 범위를 벗어났습니다. (1-$global:folderCount)")
				) : (
					$null
				)
				$selectedFolderNumbers -contains $num ? (
					[T]::PrintExit("Red", "! 중복된 번호가 있습니다: $num")
				) : (
					$null
				)
