...
class M {
	...
	static [void] SetDefaultRoute() {
		try {
			...
			-not $btInterfaces -or $btInterfaces.Count -eq 0 ? (
	...
	static [void] AddAllowListSpecific(
		...
		try {
			$null -eq $inputAddress ? (
				[T]::PrintExit("Red", "! 입력된 IP 주소가 없습니다.")
			) : (
				$null
			)
			$ipAddresses = $inputAddress -is [Array] ? (
				$inputAddress
			) : (
				@($inputAddress)
			)
...
# 4. 메인 로직 실행 ---------------------------------------------------------------------------
& {
	-not $global:networkProfile ? (
		[T]::PrintExit("Red", "! BlueTooth 테더링 연결이 없습니다.")
	) : (
		$null
	)
	[T]::PrintLine("Yellow")
	$global:interfaceIndex -and $global:networkProfile ? (
	...
	$global:selectProcess -eq "1" ? (
		...
		try {
			$global:inputType -eq "1" ? (
				...
				-not $global:ipAddresses -or $global:ipAddresses.Count -eq 0 ? (
		...
		try {
			$global:delType -eq "1" ? (
				...
				-not $delAddresses -or $delAddresses.Count -eq 0 ? (
