		...
		try {
			...
			foreach ($net in $ipAddresses) {
				...
				try {
					...
					New-NetRoute -DestinationPrefix $finalNet -InterfaceIndex $interfaceIndex -NextHop $gateway -RouteMetric $metric -Confirm:$false
				}
				catch {
	...
	static [void] RemoveAllowListSpecific(
		...
		try {
			...
			foreach ($target in $targets) {
				...
				$routes = Get-NetRoute -AddressFamily IPv4 | Where-Object { $_.NextHop -eq $global:gateway -and $_.DestinationPrefix -eq $finalTarget -and $_.InterfaceIndex -eq $global:interfaceIndex }
				foreach ($route in $routes) {
					Remove-NetRoute -DestinationPrefix $route.DestinationPrefix -NextHop $route.NextHop -InterfaceIndex $route.InterfaceIndex -Confirm:$false
				}
			}
		}
		catch {
