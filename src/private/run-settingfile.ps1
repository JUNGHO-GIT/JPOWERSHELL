...
class M {
	...
	static [array] GetProjectList() {
		...
		if (Test-Path $global:rootPath) {
			...
			foreach ($dir in $directories) {
				if ($dir.Name -ne "git") {
					$projectList += [PSCustomObject]@{ number = $number; name = $dir.Name }
					$number++
				}
			}
		}
	...
	static [array] GetCommonFileList() {
		$projectName = ($global:projectList | Where-Object { $_.number -eq $global:projectNumber }).name
		$commonPath = "$global:rootPath\$projectName\$global:commonFilePath"
		$commonFileList = @()
		if (Test-Path $commonPath) {
			$files = Get-ChildItem -Path $commonPath -File
			foreach ($file in $files) {
				$commonFileList += [PSCustomObject]@{ name = $file.Name; path = $file.FullName }
			}
		}
	...
	static [void] OverwriteCommonFiles() {
		...
		if (-not (Test-Path $listFilePath)) { [T]::PrintExit("Red", "! _list.txt 파일이 존재하지 않습니다. 경로: $listFilePath") }
		$targetPaths = Get-Content $listFilePath | Where-Object { $_.Trim() -ne "" -and -not ($_.Trim().StartsWith("#")) }
		if ($targetPaths.Count -eq 0) { [T]::PrintExit("Red", "! _list.txt에 적용할 경로가 없습니다.") }
		$overwriteCount = 0
		[T]::PrintLine("Cyan")
		...
		foreach ($targetRelPath in $targetPaths) {
			...
			elseif (Test-Path $targetAbsPath -PathType Leaf) {
				...
				if (-not $matched) { [T]::PrintText("Yellow", "▶ 공통파일에 없음(스킵): $targetAbsPath") }
			}
			else {
	...
	static [void] DeleteCommonFiles() {
		...
		if (Test-Path $deleteListFilePath) {
			$deleteList = Get-Content $deleteListFilePath | Where-Object { $_.Trim() -ne "" -and -not ($_.Trim().StartsWith("#")) }
		}
		if ($deleteList.Count -eq 0) { [T]::PrintExit("Red", "! 삭제할 파일 목록이 없습니다. 경로: $deleteListFilePath") }
		[T]::PrintLine("Cyan")
...
& {
	...
	if ($global:projectList.Count -eq 0) { [T]::PrintExit("Red", "! 프로젝트가 없습니다. 먼저 프로젝트를 생성하세요.") }
	else {
		[T]::PrintText("Yellow", "▶ 프로젝트 목록:")
		foreach ($project in $global:projectList) { [T]::PrintText("Yellow", "▶ $($project.number). $($project.name)") }
	...
	if ($global:commonFileList.Count -eq 0) { [T]::PrintExit("Red", "! 선택한 프로젝트에 공통 파일이 없습니다.") }
	else {
		[T]::PrintText("Yellow", "▶ 공통 파일 목록:")
		foreach ($file in $global:commonFileList) { [T]::PrintText("Yellow", "▶ $($file.name)") }
	}
