...
class M {
	...
	static [void] OverwriteCommonFiles() {
		...
		foreach ($targetRelPath in $targetPaths) {
			...
			Test-Path $targetAbsPath -PathType Container ? (
			...
			) : Test-Path $targetAbsPath -PathType Leaf ? (
				$fileName = Split-Path $targetAbsPath -Leaf,
				$matched = $false,
				foreach ($commonFile in $commonFileList) {
					$commonFile.name -eq $fileName ? (
...
& {
	...
	$global:commonFileList.Count -eq 0 ? (
