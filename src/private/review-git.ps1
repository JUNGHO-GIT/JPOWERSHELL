...
$global:promptTemplate = @(
	...
	"",
	"### 핵심 원칙",
	"1. **실제 동작 변경**과 **리팩토링/스타일 변경**을 명확히 구분",
	"2. **심각도 분류**: ``Critical`` / ``High`` / ``Medium`` / ``Low``",
	"3. **간결한 설명**: 핵심 이슈만 1-2문장으로 요약",
	"4. **최소한의 코드**: 수정이 필요한 경우에만, 변경된 라인만 표시 (전후 맥락 최대 2줄)",
	"",
	"### 중점 검토 항목",
	"1. 예외 처리 누락 여부",
	"2. 성능/리소스 낭비 (쿼리, 컬렉션, 루프 등)",
	"3. 불필요한 복잡도, 과한 분기, 중복 코드",
	"4. 네이밍 및 책임 분리",
	"",
	"### 코드 표시 규칙 (중요)",
	"- 문제가 없으면 코드 블록 생략",
	"- 수정 제안 시: **변경이 필요한 라인만** 표시 (최대 5줄)",
	"- 전체 메서드/함수 표시 금지",
	"- ``// ...`` 로 생략 표시",
	"",
	"---",
...
class M {
	...
	static [void] InputCommitAndOutput() {
		...
		[T]::PrintEmpty()
		$defaultFileName = "review-{0}-{1}.txt" -f $global:baseShort, $global:targetShort
		$outInput = ""
		[T]::TextInput(
			"Green",
	...
	static [void] GeneratePromptFile() {
		...
		[T]::PrintLine("Cyan")
		[T]::PrintText("Cyan", "▶ GPT 코드 리뷰용 프롬프트 파일 생성")
		...
		[T]::PrintLine("Green")
		[T]::PrintText("Green", "▶ GPT 코드 리뷰용 프롬프트 파일 생성 완료")
		[T]::PrintText("White", ("  - 파일 경로: {0}" -f $global:outputFile))
		[T]::PrintText("White", "  - 이 파일 내용을 GPT에 붙여넣으면 코드 리뷰가 생성됩니다.")
...
& {
	...
	[T]::PrintText("Cyan", "▶ Korpay Git 코드 리뷰 프롬프트 생성 시작")
}
